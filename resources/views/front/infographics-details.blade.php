...
@section('meta_keywords',!empty($infographics->meta_keyword)?$infographics->meta_keyword:'')

@section('meta_description', !empty($infographics->meta_description)?$infographics->meta_description:'')
@section('head')
<meta name='robots' content='index,follow, max-image-preview:large/' />
{!! !empty($infographics->schema_markup)?$infographics->schema_markup:'' !!}
{!! !empty($infographics->og_meta)?$infographics->og_meta:'' !!}
{!! !empty($infographics->twitter_meta)?$infographics->twitter_meta:'' !!}
@endsection
@section('content')
<style>
    .sticky-form {
            ...
            <div class="col-lg-9 col-md-9 col-sm-12 mt-lg-0">
                    <div class="iq-blog-box">
                        <div class="iq-blog-image clearfix">
                            <img src="{{!empty($infographics->image_path) && Storage::exists($infographics->image_path) ? url('/').Storage::url($infographics->image_path) : URL::asset('admin_panel/commonarea/dist/img/default1.jpg')}}" class="img-fluid" alt="qloud" />
                        </div>
                    </div>
                </div>
                               ...
                               <a href="{{url('infographics-details/'.(!empty($infographicsData->id)?$infographicsData->id:''))}}">
                                <div class="wow fadeInUp" style="visibility: visible; animation-name: fadeInUp;">
                                    <div class="icon-box-img">
                                        <img src="{{!empty($infographicsData->image_path) && Storage::exists($infographicsData->image_path) ? url('/').Storage::url($infographicsData->image_path) : URL::asset('admin_panel/commonarea/dist/img/default1.jpg')}}" class="w-100 img-fluid" alt="qloud">
                                    </div>
                                </div>
                            </a>   
                            @endforeach

...
class ReportController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'report_view')){
            ...
            $totalReportCount = Arm_reports::where('arm_reports.status', '!=', 'delete')->where('arm_report_categories.status', '!=', 'delete')->join('arm_report_categories', 'arm_report_categories.id','=','arm_reports.category_id')->count();
    ...
    public function store(Request $request){
        ...
        $input['meta_title_toc'] = $request->meta_title_toc;
        $input['meta_keyword_toc'] = $request->meta_keyword_toc;
        $input['meta_description_toc'] = $request->meta_description_toc;
        $input['meta_title_rm'] = $request->meta_title_rm;
        $input['meta_keyword_rm'] = $request->meta_keyword_rm;
        $input['meta_description_rm'] = $request->meta_description_rm;
        $input['meta_title_rfsp'] = $request->meta_title_rfsp;
        $input['meta_keyword_rfsp'] = $request->meta_keyword_rfsp;
        $input['meta_description_rfsp'] = $request->meta_description_rfsp;
        ...
        if(!empty($id)){
            ...
            if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'report_add')){
                ...
                if($response = Arm_reports::create($input)){
                    $prefix = "WMR_";
                    $suffix = sprintf("%07s", $response->id);
    ...
    public function report_data_table(Request $request){
        ...
        $reports = $reports->select('arm_reports.id', 'arm_reports.report_id', 'arm_reports.title', 'arm_reports.publisher', 'arm_reports.url', 'arm_reports.published_on','arm_reports.category_id', 'arm_report_categories.category_name', 'arm_reports.status', 'arm_reports.top_selling', 'arm_reports.upcoming_report')->get();
        ...
        if ($request->ajax()) {
            return DataTables::of($reports)
                ...
                ->rawColumns(['action','status','top_selling_report', 'url', 'title', 'related_report'])
                ->make(true);
        }
    }
    ...
    public function report_export(Request $request){
        ...
        $RolesPrivileges = Arm_role_privilege::where('id', $role_id)->where('status', 'active')->select('privileges')->first();
        if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'report_other')){
            $all_reports = Arm_reports::where('arm_reports.status', '!=', 'delete')->where('arm_report_categories.status', '!=', 'delete')->join('arm_report_categories', 'arm_report_categories.id','=','arm_reports.category_id')->orderBy('arm_reports.id', 'desc');
            if(!empty($request->category_id)){
    ...
    function store_published_date(){
        $all_reports = Arm_reports::get()->take(2);
        foreach($all_reports as $report){
            $date = date('d/m/Y',strtotime($report->published_on));
            Arm_reports::where('id',$report->id)->update(array('published_on_1' => $date));
        }
    }

    // public function store_report_url(){
    //     function extractSentence($inputString) {
    //         $words = explode(' ', $inputString);
    //         if (isset($words[0]) && $words[0] === 'Global') {
    //             unset($words[0]);
    //             $words = array_values($words);
    //         }
    //         $marketIndex = array_search('Market', $words);

    //         if ($marketIndex !== false) {
    //             $resultString = implode(' ', array_slice($words, 0, $marketIndex + 1));
    //             return $resultString;
    //         }
    //         return $inputString;
    //     }
    //     // $input['url'] = Str::slug(extractSentence("Automotive Conformal Coatings Market Size, Industry Analysis Report by Type(Acrylics, Urethanes)"));
    //     // dd($input['url']);

    //     $all_reports = Arm_reports::get();
    //     foreach($all_reports as $report){
    //         $input['url'] = Str::slug(extractSentence($report->title));
    //         // dd(Str::slug(extractSentence($report->title)));
    //         $report = Arm_reports::where('id', $report->id)->update($input);
    //     }
    //     dd("success");
    // }
    ...
    public function store_report_id(){
        ...
        foreach($all_reports as $report){
            $prefix = "WMR_";
            $suffix = sprintf("%07s", $report->id);
            Arm_reports::where('id', $report->id)->update(array("report_id" => $prefix.$suffix));
        }
        dd("success");
    }
}

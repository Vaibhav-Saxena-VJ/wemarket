...
class FrontPaypalController extends Controller
{
    public function index(Request $request){
        if($request->license != "undefined"){
            if($request->license == "single" || $request->license == "multi" || $request->license == "enterprise" ){
    ...
    public function payment(Request $request)
    {
            ...
            if(isset($response['id']) && $response['id'] != null){
                foreach($response['links'] as $link){
                    if($link['rel'] == "approve"){
                        ...
                        try{
                            Mail::to($this->sales_email())->send(new mailToAdminBeforePayment($input));
                        } catch (Throwable $e) {
                            return true;
                        } 
        ...
        if(isset($request->payment_method) && $request->payment_method == "stripe"){
            ...
            if($response->status == "open"){
                ...
                Arm_payment_detail::create($input);
                try{
                    Mail::to($this->sales_email())->send(new mailToAdminBeforePayment($input)); // Mail to Admin
                } catch (Throwable $e) {
                    return true;
                } 
            }
    ...
    public function paypal_success(Request $request){
        ...
        if(isset($order_details['status']) && $order_details['status'] == 'COMPLETED'){
            ...
            try{
                Mail::to($this->sales_email())->send(new mailToadminAfterPayment($user_inputs)); // Mail to Admin
    ...
    public function payment_cancel(){
        $user_inputs = Session::get('user_inputs');
        Mail::to($this->sales_email())->send(new mailToAdminAfterPaymentCancel($user_inputs)); // Mail to Admin

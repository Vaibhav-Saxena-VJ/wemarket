...
class FrontReportController extends Controller
{
    ...
    public function category_wise_report($category_slug, $category_id, Request $request){
        ...
        if(empty($category_details) || ($category_slug != Str::slug(Str::replace('&','and',$category_details->category_name) , '-'))){
            return redirect('/404');
        }
    ...
    public function report_details($report_slug, $report_id){
        $report_url = Arm_reports::where('status','=','active')->where('url',$report_slug)->first();
        if(empty($report_url)){
            return redirect('/404');
        }
